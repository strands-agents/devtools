...
class ShellSession:
    ...
    def run(self, command: str, timeout: int | None = None) -> str:
        ...
        with self._run_lock:
            if not self._alive or not self.process or self.process.poll() is not None:
...
def shell_tool(
    ...
    **Supported commands:**
    - Standard shell commands: cd, pwd, ls, cat, git, etc.
    - Build/test commands: pytest, npm, make, etc.
    ...
    **Unsupported/unreliable:**
    ...
    """
    SESSION_ATTR = '_shell_tool_session'
    ...
    if not hasattr(tool_context.agent, SESSION_ATTR):
        setattr(tool_context.agent, SESSION_ATTR, ShellSession())

    session = getattr(tool_context.agent, SESSION_ATTR)
